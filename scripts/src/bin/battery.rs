...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if Path::new(&state_file).exists() {
        ...
        if Path::new("/sys/class/power_supply/BAT0").exists() {
            match mode {
                1 => {
                    let Ok((battery_percent, battery_state, color)) = show_battery() else {
                        todo!()
                    };
                    if battery_state == "Charging" {
                        ...
                        println!(
                            ...
                            color,
                            battery_percent,
                            state[(battery_percent / 20).min(4)]
                        );
                    }
                }
                2 => {
                    ...
                    if status == "Charging" || status == "Not charging" {
                        ...
                        color = &color_battery;
                        let Ok((remaining_hours, remaining_minute)) = show_remaining_time() else {
                            todo!()
                        };
                        println!(
                            "<span foreground='{}'>{} hours {} remaining</span>",
                            color, remaining_hours, remaining_minute
                        )
                    }
                }
                _ => {}
            ...
            println!(
                "<span foreground='{}'>Batterie sur secteur</span>",
                color_full
            );
        }
    } else {
